...
{ 
	...
	if (anum < INT32_MIN)
		throw exception("Значение числителя слишком маленькое! Минимально возможное: -2147483647");
	num = anum;
	reduction();
}
...
void fraction::Setden(long long aden)
{
	...
	if (aden < INT32_MIN)
		throw exception("Значение знаменателя слишком маленькое! Минильно возможное: -2147483647");
	if (aden == 0)
...
fraction fraction::operator + (const fraction& p) const  // Реализация сложения 
{
	fraction x;
