...
namespace lab2
{
    ...
    {
        public static double GetDispersion(this List<int> sequence,double mathExpectation) // сумма всех значений
        {
            var sum = sequence.Sum(seq => (seq - mathExpectation) * (seq - mathExpectation));
            return  sum / (sequence.Count - 1);
        }

        public static double GetMathExpectation(this List<int> sequence) // математическое ожидание
        {
            var sum = sequence.Aggregate<int, double>(0, (current, seq) => current + seq);
            return sum/sequence.Count;
        }
        
        public static double CriterionPearsonBernoulli(this List<int> sequence, double p) // критерий пирсона биноминальный 
        {
        ...
        public static double CriterionPearsonGeometric(this List<int> sequence, double p) // критерий пирсона геометрический
        {
            var n = sequence.Count;
            double xi = 0;
        ...
        public static double CriterionPearsonBinomial(this List<int> sequence, params double[] param) // критерий пирсона биноминальный
        {
            var m = (int) param[0] + 1;
            var p = param[1];
            var n = sequence.Count;
        ...
        public static double CriterionPearsonNegBinomial(this List<int> sequence, params double[] param) // критерий пирсона отрицательный биноминальный
        {
            var r = (int) param[0];
            var p = param[1];
            var n = sequence.Count;
        ...
        private static long Cominations(int allNumbers, int perGroup) // число попаданий
        {
