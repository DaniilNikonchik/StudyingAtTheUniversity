...
public class Index implements Serializable, Closeable {
    ...
    IndexOne2One brands;
    IndexOne2N models;
    IndexOne2One registrationNumbers;

    public void test(Car car) throws KeyNotUniqueException {
        assert (car != null);
        if (brands.contains(car.brand)) {
            throw new KeyNotUniqueException(car.brand);
        }
        if (registrationNumbers.contains(car.registrationNumber)) {
            throw new KeyNotUniqueException(car.registrationNumber);
        }
    }

    public void put(Car car, long value) throws KeyNotUniqueException {
        test(car);
        brands.put(car.brand, value);
        models.put(car.model, Car.authorDel, value);
        registrationNumbers.put(car.registrationNumber, value);
    }

    public Index() {
        brands = new IndexOne2One();
        models = new IndexOne2N();
        registrationNumbers = new IndexOne2One();
    }
